...
\[
    \vec{r} = \left\lgroup\diff x = h; \diff y = y'\diff x = -\frac{f_{x}}{f_{y}}\diff x\right\rgroup^{tr}
\]


\subsection{Gradientenfeld \texorpdfstring{$\perp$}{\_|\_} Kontouren}
\[
    \nabla f \tikznode{scprd}{\bullet} \begin{pmatrix}
        \diff x\\
        \diff y = y'\diff x
...
\begin{ctabular}{r l}
    $s(t) :$ & $P_0 + t \cdot \hat{v} \mid t\in \mathbb{R}$\\
    $s(t) :$&$f(x_0 + t \cdot \hat{v}_1\text{ };\text{ }y_0 + t \cdot \hat{v}_2)$\\
    &\\
    $\frac{\text{d }s(t)}{\text{d}t}=\dot{s}(t) :$&$t\mapsto\overbrace{\begin{pmatrix}x_0+t\cdot v_1\\y_0+t\cdot v_2\end{pmatrix}}^{\begin{pmatrix}x\\y\end{pmatrix}}\mapsto f(x,y)$
\end{ctabular}

\subsection{Richtungs-Ableitung}
\[
    \frac{\partial f}{\partial\hat{v}}\overset{!}=D(f;(x_{0};y_{0}))\cdot\hat{v}\overset{Def.}\Leftrightarrow grad(f)^{tr}\cdot\hat{v}=f_{x}\cdot v_{1}+f_{y}\cdot v_{2}
\]
...
\[
    \frac{\partial f}{\partial\hat{e}_{1}}=f_{x}\cdot1+f_{y}\cdot0=\uuline{f_{x}}
\]