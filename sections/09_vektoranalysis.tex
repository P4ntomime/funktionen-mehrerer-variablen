...
Der Gradient eines Vektorfeldes $\vec{V}: \mathbb{R}^n \to \mathbb{R}^m$ lässt sich durch folgende Definition
\[ \frac{\partial \vec{V}}{\partial \vec{a}} = \vec{a} \dotp \grad \vec{V}, \]
wobei $\vec{a}$ ein beliebiger Vektor und $\frac{\partial \vec{V}}{\partial \vec{a}}$ die Richtungsableitung des Vektorfelds $\vec{V}$ nach $\vec{a}$ ist, als
\[ 
    \grad \vec{V} 
    = \nabla \vec{V}
    = \begin{pmatrix}
    ...
    \end{pmatrix} 
    = \mathbf{J}
    \hspace{1em} \left(= \nabla \dotp \vec{V}^T\right)
\]
berechnet werden.
\begin{itemize}
    \item $\nabla \vec{V}$ entspricht der Jacobi-Matrix aus Kapitel *TODO: Ref einfügen*.
    \item Der Vektorgradient wird als $\nabla \vec{V}$ geschrieben, da sonst Verwechslungsgefahr mit der Divergenz $\nabla \dotp \vec{V}$ besteht.
\end{itemize}
